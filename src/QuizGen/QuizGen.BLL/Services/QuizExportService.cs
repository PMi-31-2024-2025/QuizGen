...
public class QuizExportService : IQuizExportService
{
    ...
    public async Task<ServiceResult<byte[]>> ExportAsTextAsync(int quizId, bool includeAnswers)
    {
        try
        {
            ...
            // Add quiz header
            _ = sb.AppendLine($"{quiz.Name}");
            _ = sb.AppendLine($"Topic: {quiz.Prompt}");
            _ = sb.AppendLine($"Difficulty: {char.ToUpper(quiz.Difficulty[0]) + quiz.Difficulty[1..]}");
            _ = sb.AppendLine($"Created: {quiz.CreatedAt:g}");
            _ = sb.AppendLine();
            ...
            for (int i = 0; i < questions.Count(); i++)
            {
                ...
                _ = sb.AppendLine($"{i + 1}. {question.Text}");
            ...
            _ = sb.AppendLine("Generated by QuizGen");
    ...
    public async Task<ServiceResult<byte[]>> ExportTryAsTextAsync(int quizTryId)
    {
        try
        {
            ...
            _ = sb.AppendLine($"Difficulty: {char.ToUpper(quiz.Difficulty[0]) + quiz.Difficulty[1..]}");
            _ = sb.AppendLine();
            _ = sb.AppendLine($"Taken on: {quizTry.StartedAt:g}");
            _ = sb.AppendLine($"Score: {quizTry.Score:F1}%");
            _ = sb.AppendLine();
